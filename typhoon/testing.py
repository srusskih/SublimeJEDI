...
* `main()`: A simple test runner (wrapper around unittest.main()) with support
  for the typhoon.autoreload module to rerun the tests when code changes.
"""

from __future__ import absolute_import, division, with_statement

try:
    from typhoon import gen
    from typhoon.httpclient import AsyncHTTPClient
    from typhoon.httpserver import HTTPServer
    from typhoon.simple_httpclient import SimpleAsyncHTTPClient
    from typhoon.ioloop import IOLoop
    from typhoon import netutil
except ImportError:
...
from typhoon.log import gen_log
from typhoon.stack_context import ExceptionStackContext
from typhoon.util import raise_exc_info, basestring_type
import functools
import logging
import os
import re
...
# Typhoon's own test suite requires the updated unittest module
# (either py27+ or unittest2) so typhoon.test.util enforces
# this requirement, but for other users of typhoon.testing we want
...
class AsyncTestCase(unittest.TestCase):
    ...
    Example::
        ...
        class MyTestCase2(AsyncTestCase):
            def test_http_fetch(self):
                client = AsyncHTTPClient(self.io_loop)
                client.fetch("http://www.typhoonweb.org/", self.stop)
                response = self.wait()
                # Test contents of response
                self.assertIn("FriendFeed", response.body)
        ...
        class MyTestCase3(AsyncTestCase):
            def test_http_fetch(self):
                client = AsyncHTTPClient(self.io_loop)
                client.fetch("http://www.typhoonweb.org/", self.handle_fetch)
                self.wait()
...
class AsyncHTTPTestCase(AsyncTestCase):
    ...
    `typhoon.web.Application` (or other `.HTTPServer` callback) to be tested.
    ...
    def get_app(self):
        ...
        `typhoon.web.Application` or other `.HTTPServer` callback.
        """
...
class AsyncHTTPSTestCase(AsyncHTTPTestCase):
    ...
    def get_ssl_options(self):
        ...
        # openssl req -new -keyout typhoon/test/test.key -out typhoon/test/test.crt -nodes -days 3650 -x509
...
class LogTrapTestCase(unittest.TestCase):
    ...
    `typhoon.options`.  It is not compatible with other log buffering
...
class ExpectLog(logging.Filter):
    ...
    Usage::
        ...
        with ExpectLog('typhoon.application', "Uncaught exception"):
...
def main(**kwargs):
    ...
    the standard library, but adds support for typhoon-style option
    parsing and log formatting.

    The easiest way to run a test is via the command line::

        python -m typhoon.testing typhoon.test.stack_context_test

    See the standard library unittest module for ways in which tests can
    be specified.

    Projects with many tests may wish to define a test script like
    ``typhoon/test/runtests.py``.  This script should define a method
    ``all()`` which returns a test suite and then call
    `typhoon.testing.main()`.  Note that even when a test script is
    used, the ``all()`` test suite may be overridden by naming a
    single test on the command line::

        # Runs all tests
        python -m typhoon.test.runtests
        # Runs one test
        python -m typhoon.test.runtests typhoon.test.stack_context_test

    Additional keyword arguments passed through to ``unittest.main()``.
    For example, use ``typhoon.testing.main(verbosity=2)``
    to show many test details as they are run.
    See http://docs.python.org/library/unittest.html#unittest.main
    for full argument list.
    """
    from typhoon.options import define, options, parse_command_line
